...
export class ScheduleDay extends Component {
    ...
    render() {
        ...
        const overflowBool = ((this.props.timedeltas.length > 3) && smallestMedia) || ((this.props.timedeltas.length > 5) && smallMedia && !smallestMedia) || (this.props.timedeltas.length > 3 && !smallMedia);
        const boxPleft = overflowBool ? { paddingLeft: "12px" } : { paddingLeft: "0px" };
        const theadRowWidth = overflowBool ? { display: "table", tableLayout: "fixed", width: "calc(100% - 12px)" } : { display: "table", tableLayout: "fixed", width: "100%" };
